...
const verifyToken = async (req, res, next)=>{
    try{
        ...
        if(decoded){ req.user_id = decoded.user_id; next();}
...
const createToken = async (req,res,next) => {
    try{
        ...
        const exist = await pool.query("SELECT id FROM public.user WHERE user_id = $1", [req.body.user_id]);
        const password = await pool.query("SELECT password FROM public.user WHERE user_id = $1", [req.body.user_id]);
        ...
        try{
        if(ex && validPassword){
            const token = jwt.sign({user_id: ex}, YOUR_SECRET_KEY,{expiresIn: '1h'});
...
const signUp = async(req,res) => {
    try{ const result = await createUser(req.body);
